...
def get_recent_deliveries(
    ...
    """
    Get recent delivery history for context
    ...
    Returns:
        DataFrame with recent deliveries
    """
    try:
        engine = get_db_engine()
        ...
        SELECT 
            dn_number,
            DATE(sto_etd_date) as etd_date,
            stock_out_request_quantity as quantity,
            shipment_status,
            customer
        FROM delivery_full_view
        WHERE product_id = :product_id
